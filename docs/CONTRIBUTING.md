...
* **Trait object bounds are sorted**. For example, `dyn Foo + Sync + Send` is rendered as `dyn Foo + Send + Sync`. Avoids issues like [Stable/predictable sort order needed for implemented traits](https://github.com/cargo-public-api/cargo-public-api/issues/907).
